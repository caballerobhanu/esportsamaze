      ...
      {open && (
        ...
        >
          ...
          {filtered.length === 0 &&
            (freeText ? (
              <li
                className="px-3 py-2 text-[#0A5FC4] font-semibold cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800"
                onMouseDown={(e) => {
                  e.preventDefault();
                  commit(query.trim());
                }}
  ...
  return (
    <li
      ...
      className={
        ...
        (active ? 'bg-[#0A5FC4]/10 dark:bg-[#0A5FC4]/20' : '')
      }
    >
      <span className={selected ? 'font-bold text-[#0A5FC4] dark:text-blue-400' : ''}>{label}</span>
      {selected && <span className="text-[10px] font-bold uppercase text-[#0A5FC4] dark:text-blue-400">✓</span>}
    </li>
  );
}
