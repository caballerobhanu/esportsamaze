...
import {
  ...
  mergePlayersAction,
  togglePlayerVerificationAction,
  batchVerifyPlayersAction,
...
export function PlayersManagerTable({ players, deletePlayerAction }: PlayersManagerTableProps) {
  ...
  // Delete modal state
  const [playerToDelete, setPlayerToDelete] = useState<{ id: string; ign: string } | null>(null);
                  ...
                  <td className="py-2.5 px-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        onClick={() => {
                          startTransition(async () => {
                            const res = await duplicatePlayerAction(p.id);
                            if (res.success && res.newPlayerId) {
                              window.location.href = `/admin/players?edit=${res.newPlayerId}&saved=copy`;
                            } else {
                              alert(res.message || 'Failed to duplicate player');
                            }
                          });
                        }}
                        className="p-1 rounded-md hover:bg-emerald-50 dark:hover:bg-emerald-950/40 text-slate-400 hover:text-emerald-600 transition-colors cursor-pointer"
                        title={`Duplicate "${p.ign}"`}
                        aria-label={`Duplicate "${p.ign}"`}
                      >
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0b101c] p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Are you sure you want to delete <strong className="text-slate-900 dark:text-white">&quot;{playerToDelete.ign}&quot;</strong>?
                </p>
              </div>
            </div>

            <p className="text-xs text-slate-500 bg-slate-50 dark:bg-slate-900 p-3 rounded-lg leading-relaxed">
              This will delete the player profile and clean up any linked transfers or statistics. This action cannot be undone.
            </p>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              ...
              <button
                ...
                onClick={() => {
                  startTransition(async () => {
                    const res = await deleteSinglePlayerAction(playerToDelete.id, true);
                    if (res.success) {
                      setPlayerToDelete(null);
                    } else {
                      alert(res.message || 'Failed to delete player');
                    }
                  });
                }}
