...
export async function generateMetadata({
  ...
  return {
    ...
    ...canonical(gameHref(game, 'rankings')),
  };
}
...
export default async function RankingsPage({
  ...
  const { game } = await routeParams;
  // See generateMetadata: rankings exist only for the game that owns the board.
  ...
  const boardPath = isPlayers
    ? `${gameHref(game, 'rankings')}?board=players`
    : gameHref(game, 'rankings');
  ...
  const rankingJsonLd = rankedItemListJsonLd(
    ranked.map((entry) => {
      ...
      return {
        ...
        url: slug ? absoluteUrl(gameHref(game, `rankings/${isPlayers ? 'player' : 'team'}/${slug}`)) : null,
      };
    }),
